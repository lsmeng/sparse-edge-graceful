...
Section~\ref{sec:cells}; our constructor carries it separately.  The two
direct-edge two-input rows are a genuine exception, since their heads are
determined by their two inputs, and a chain of them does propagate a symbolic
head upward.  We have not bounded the length of such a chain, so the bound
$3W$ below should be read as $3W$ plus $W$ for each level of that chain; in our
regression the chains have length zero, and we record the bound as the second
place where our proof is incomplete.
