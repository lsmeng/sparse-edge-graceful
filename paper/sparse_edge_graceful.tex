...
solution expresses $K(F)$ as affine functions of $\Lambda$.  The equation is
symmetric and may instead be solved for $\Lambda$, which
Remark~\ref{rem:integrality} uses.  Writing the two
...
The carrier is ours to choose, and two choices remove the difficulty for
different reasons.  A \emph{unimodular} carrier leaves nothing to divide.  A
carrier avoiding the head leaves the head an independent parameter, so the
value the cell passes to its parent is one we choose freely and no coarser
lattice travels upward.  Call a family \emph{safe} when each of its tokens
admits one or the other.  Two further observations cut the problem down.
First, only a cell that resolves a pending token performs the division, and
such a cell has the pending cell in its subtree, hence has an active child; a
bottom cell has only arms and ports below it and can never divide.  Second, the
catalogue may be enlarged: we searched for safe families for the contexts that
lacked one and found six, each with a unimodular carrier.

After that, $2{,}770$ of the $\NumEsixDenom$ contexts that occur have a safe
family, and exactly four contexts can divide and have none, namely
$\mathtt{h0\_3\_p0\_act}$, $\mathtt{h0\_5\_p0\_act}$,
$\mathtt{h1\_2\_p0\_act}$ and $\mathtt{h4\_1\text{-}3\text{-}3\text{-}3\_p0\_act}$.
For the first three this is not a limitation of our search.  Their widths are
six, eight and six, so every bijection between labels and outputs can be
enumerated, and we did: solving $P'=O$ for each gives $3$, $73$ and $3$
families whose labels are distinct and nonzero, and after writing each over the
basis in which the head is $y$ and the input is $x$, not one of them has a safe
token.  In all three the token spans a plane on the pair $(y,x)$ alone, with
$|\det|$ equal to $3$, $3$ and $6$.  A token cannot be avoided either, since
the parity of their non-head label count forces one, and their own numeric
parameters cannot clear the determinant, because a single non-carrier column
generates a subgroup of $(\Z/\det)^2$ of rank one.  The fourth has width
twenty and is beyond enumeration.

One more freedom halves what is left.  The pairing equation $T_v=-T_A$ is
symmetric in its two cells: it is two independent equations in the two carrier
pairs, and we may solve it for the carrier of the pending cell $A$ as readily
as for that of the resolving cell $v$, since (E4) makes the coefficient matrix
of $T_A$ on its own carrier nonsingular too.  Solving for $A$'s carrier
expresses $\Lambda$ in terms of $K(F_v)$, which is then chosen numerically, and
the division is by $\det$ of $A$'s carrier block instead.  We therefore take
whichever of the two sides is safe.  Of the $1{,}724$ contexts that occur and
can carry a token, and so can be either the pending cell or the resolving one,
exactly the four above are unsafe, and a resolution is now in difficulty only
when \emph{both} of its cells are among those four.

What is still missing for that case is a bound on the number of such
eliminations along a root path that pass a value to one another, or a joint for
these four of the kind Lemma~\ref{lem:reset} supplies for the two-input row.
Until one of the two is given, the explicit constant $A$ below is not
established for a tree in which two of the four meet in this way, although our
constructor produces integer labellings on every tree we have run it on.
