...
Theorem~\ref{thm:linear} rests on the linear deletion bound for cyclic groups
of \cite{MuyesserPokrovskiy2025} through Lemma~\ref{lem:dense}
(Remark~\ref{rem:mplinear}).  Here we reduce
Lemma~\ref{lem:dense} to a single statement about permutations of an interval,
prove that statement at the two ends of its range and for a family of composite
orders, and verify it exactly up to order $30$.  A proof in general would give
Theorem~\ref{thm:linear} unconditionally, with an explicit $C$.
...
$(1-t)/2\le s\le(t+1)/2$, and beyond $K\ge7M+3$ Simpson's theorem~\cite{Simpson1983} on Langford
...
or two empty cells, verified for $M\le30$).  Together with Simpson's theorem
on Langford sequences, which covers $K\ge7M+3$ with no folding, this gives
Lemma~\ref{lem:dense} with
$\eta=1/(14A+O(1))$, hence Theorem~\ref{thm:linear} unconditionally, with an
explicit $C$ and without any use of \cite{MuyesserPokrovskiy2025}.  The punctured
