...
The carrier is ours to choose, and two choices make the difficulty disappear
for different reasons.  A \emph{unimodular} carrier leaves nothing to divide.
A carrier avoiding the head leaves the head an independent parameter, so the
value the cell passes to its parent is one we choose freely and no coarser
lattice travels upward.  A family is safe if each of its tokens admits one or
the other, and $2{,}764$ of the $\NumEsixDenom$ contexts that occur have such a
family.

Of the $25$ that do not, fifteen are bottom cells.  A bottom cell has no active
child, so no cell of the decomposition lies below it, so it can never be the
cell that resolves a pending token, and the elimination never takes place
there.  That leaves ten, all of them with a free active child; we have found
safe families for three of them by a wider search.  For the remaining seven the
obstruction is sharp and worth recording: the token of every family in their
menus spans a plane on exactly one pair of parameters, the head and the input,
with $|\det|\in\{2,3,4,6\}$, and every other pair gives a vanishing minor.  A
token cannot be avoided either, since the parity of their non-head label count
forces one.  Their own numeric parameters cannot clear the determinant, because
the reachable subgroup of $(\Z/\det)^2$ is generated by one vector.  So for
these seven the eliminated pair is the head and the input, the lattice of the
head is divided by at most six, and what is still missing is a bound on the
number of eliminations along a root path that pass a value to one another.
Until that is supplied the explicit constant $A$ below is not established for
trees that use one of these seven contexts, although our constructor produces
integer labellings on every tree we have run it on.
