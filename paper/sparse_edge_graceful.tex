...
propagation can start only at the remaining $147$.  We record the consequence
plainly.  The bound $3W$ below is correct only when no such chain is live, and
in general it becomes $(3+L)W$ with $L$ the length of the chain, so that
$|\Phi|$ and the boxes are $O(D^2)$ rather than $O(D)$ and the magnitudes are
$O((D+1)^2)$.  Our theorems are stated with a threshold linear in $D$, and that
form is therefore not established by the present argument; what the argument
gives unconditionally is the same statement with $(D+1)$ replaced by
$(D+1)^2$.  Closing this needs a reset for the residual-$\{1\}$ two-input row,
of the kind the residual-$\{2\}$ row turns out not to need, and we have not
found one.
