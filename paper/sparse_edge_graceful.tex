...
lattice, so at this step we choose $\Lambda\in(dM\Z)^{|\Lambda|}$ with
$d=|\det M|$ rather than in $(M\Z)^{|\Lambda|}$; then $N\Lambda+c$ is divisible
by $d$ and $K$ is again integral and in $M\Z$.  Since $d\le\NumDetMax$, the
refined lattice is contained in $M'\Z$ with $M'=M\cdot\NumDetMax=\NumLatticeMEff$,
and the only cost is that the box grows by the factor $d$.  Second, a label
...
the new conditions, and choose $\Lambda$ in that lattice avoiding all
conditions in $\Phi$.  Each condition is a proper affine subspace, so a lattice
point exists in a box of side $M'(|\Phi|+1)$.  We note that the enlargement
does not compound: at the end of this step nothing is pending, so the next
pending token starts again from catalogue coefficients bounded by
$\NumCoefBound$, and the factor is paid once and not once per owner.  All labels of $A$, of its child
