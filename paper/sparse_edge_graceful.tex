...
\date{Draft of 2 September 2026}
...
we recently settled the case of at most two such vertices.  We
prove that there are absolute constants $C$ and $n_0$ such that every tree of
odd order $n\ge n_0$ with $D$ vertices of degree two is edge-graceful whenever
$n>C(D+1)$.  The proof combines a finite table of local labelling identities,
found by an exact constraint search and re-verified by an independent checker,
with a bottom-up scheduling argument and with the linear-deficiency zero-sum
matching of M\"uyesser and Pokrovskiy.  The linear deficiency is the
improvement those authors record for abelian groups in their Section~7.4.  The
printed form of their theorem gives the same conclusion with
$n>C(D+1)(\log n)^{A}$, and in particular gives, for every fixed $k$, that
every tree of odd order that is large enough and has at most $k$ vertices of
degree two is edge-graceful.  The constants are not effective.
\end{abstract}
...
Equivalently, for every $k\ge 0$ every odd-order tree with at most $k$ vertices
of degree two and $n>2Ck$ vertices is edge-graceful, the cases $k=0$ and
$k\le 2$ being the theorems of \cite{KaplanLevRoditty2009} and
\cite{Meng2026two}, respectively.  However, the constants come from an
asymptotic matching theorem and are therefore not effective.
Theorem~\ref{thm:main} also says nothing about trees in which the degree-two
vertices have positive density, e.g.\ caterpillars with many subdivided
edges, the regime of the full conjecture.

The only external input to Theorem~\ref{thm:main} is a zero-sum matching
theorem in $\Z_n$ that tolerates a linear number of deleted residues, and it
is the linear tolerance, and not the matching itself, that is delicate.
The printed statements of M\"uyesser and Pokrovskiy~\cite{MuyesserPokrovskiy2025}
tolerate $n/(\log n)^{A}$ deletions.  However, their Section~7.4 records that
for abelian groups the logarithms may be removed, since the only source of a
logarithm in their proof is a bound on the length of a commutator product and
that product is empty in an abelian group.  Theorem~\ref{thm:main} uses the
latter reading, which the authors state and use themselves.  Everything else in
our proof is finite or elementary, and using only the printed statements it
gives the following.
...
Thus the qualitative statement, that a bounded number of degree-two vertices
is no obstruction once the tree is large, does not depend on the remark of
Section~7.4 at all.  The remark is what turns the polylogarithmic threshold
into a linear one.
...
sizes by the linear-deficiency form of the random Hall--Paige theorem of
M\"uyesser and Pokrovskiy~\cite{MuyesserPokrovskiy2025}.  Therefore the local part is a
...
proves Theorem~\ref{thm:main}.  Section~\ref{sec:verification} describes the
...
settled it by an exhaustive search; the resulting labels are listed in
...
instead of one and coefficients up to $14$ in absolute value, so the six
labels of a $\{1,3\}$ block cannot be made to pay for themselves however they
are written, and the pair is in any case the one our construction singles out
everywhere else as well.  A $\{1,3\}$ block can still be carried
...
children.  We therefore enumerated all $314{,}280$ abstract local states with up to
...
\section{Dense completion and the proof of Theorem~\ref{thm:main}}\label{sec:dense}
...
gives Theorem~\ref{thm:polylog}.  Their Section~7.4 records that
...
density and gives Lemma~\ref{lem:dense} as stated.  This
improvement is described by the authors as requiring essentially no
modification of their proof, although it is not a printed theorem.  The
constant $\eta$ is in either case inherited from their argument and is not
effective.
...
with $|S|\le 12D+3$ and magnitudes at most $A(D+1)$, place the pending triple,
and complete by Lemma~\ref{lem:dense}, which applies when $2A(D+1)+1\le\eta n$.
Every stationary vertex then has a zero-sum block, every special vertex has the
prescribed sum, and Lemma~\ref{lem:sigma} gives the bijection.  Taking
$C\ge 3A/\eta$ and $n_0\ge n_1$ completes the proof.  Theorem~\ref{thm:polylog}
is the same argument with the printed perturbation radius of
\cite{MuyesserPokrovskiy2025} in place of the linear one, so that the
hypothesis $2A(D+1)+1\le\eta n$ becomes $2A(D+1)+1\le n/(\log n)^{A'}$.  The
final assertion follows by taking $D\le k$ and $n$ large.
\end{proof}
...
on $119$ of $120$ random trees.  In its \emph{proof-faithful} mode it
...
trees it succeeded on $105$.  Of the fifteen remaining runs, nine report a
modular collision between two special labels, or a pending triple whose
negatives are already used modulo $n$, and six report a failure of the
completion step.  We note that these runs use the dense completion only
...
tokens were pending, and the largest label magnitude was about $22(D+1)$,
against the bound $A=\AConst$ that we prove.
...
linear deficiency would therefore make Theorem~\ref{thm:main} effective
without any change to the construction.  However, the degree-two vertices
...
\paragraph{Use of AI.}
Anthropic's Claude was used as a coding and analysis assistant throughout this
project.  It implemented and executed the search, verification, scheduling and
construction code, prepared documentation, surveyed literature, and assisted in
preparing manuscript text.  The scientific content is the author's own, and the
manuscript was written or rewritten, edited and approved by the author.  This
paper contains no figures, and its two tables are generated directly by the
analysis code from the certificate catalogue; no figure, schematic or
illustration was produced by a generative image model.  All scientific
questions, study design decisions and interpretations are the author's.  Every
computational claim was re-run by the author from the scripts in the
repository, and the independent checker was written without access to the
verification code of the search.
...
Section~\ref{sec:verification} are available at \todo{repository URL}.
